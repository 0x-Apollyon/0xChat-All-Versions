...
def broadcast_all(data):
    ...
    if sym_encryption:
        for client in client_list:
            ...
            enc_msg = client_cipher_rsa.encrypt(data.encode())
...
if sym_encryption:
    ...
    if encryption_algo.lower() == "rsa":
        rsa_public_key_path = os.path.join(server_utils_path , "rsa_public_key.aplyn")
        rsa_private_key_path = os.path.join(server_utils_path , "rsa_private_key.aplyn")
...
server_info_msg = f"--SERVER-INFO--|SECUREMODE:{sc_mode}|SYM-ENCRYPTION:{sm_enc}|RSA-KEY:{rsa_public_key_raw}"
...
while True:
    if not sym_encryption:
        ...
        client_list.append((conn,client_cipher_rsa))
        threading.Thread(target=handle_client, args=(conn , caddr,)).start()
